...
(define (domain logistics)
  ...
  (:action load-truck
    :parameters (?pkg - package ?truck - truck ?loc - hub)
    :precondition (and
      (at ?truck ?loc)
      (at ?pkg ?loc)
    )
  ...
  ; A van can always drive between locations inside cities
  (:action drive-van
  ...
  ; A truck can move between connected cities through hubs
  (:action drive-between-hubs
    :parameters (?truck - truck ?fromHub - hub ?toHub - hub ?fromCity - city ?toCity - city)
    :precondition (and
      (at ?truck ?fromHub)
