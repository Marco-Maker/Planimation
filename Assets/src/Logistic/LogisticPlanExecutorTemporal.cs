...
public class LogisticPlanExecutorTemporal : MonoBehaviour
{
    ...
    IEnumerator ExecuteAction(string action)
    {
        ...
        switch (parts[0])
        {
            ...
            case "refueling":
                yield return RefuelTruck(parts[1], parts[2]);
                break;
    ...
    IEnumerator RefuelTruck(string truckName, string station)
    {
        GameObject truck = FindObject(vehicles, truckName);
        if (truck == null || !places.ContainsKey(station)) yield break;

        Vector3 stationPos = places[station].transform.position;
        stationPos.y = truck.transform.position.y;

        Debug.Log($"Refueling {truckName} at {station}");
        yield return MoveToPosition(truck, stationPos);
        yield return new WaitForSeconds(3f); // dura sempre 3 nel dominio
    }
